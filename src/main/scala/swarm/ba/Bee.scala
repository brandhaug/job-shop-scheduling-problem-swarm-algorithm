...
case class Bee(positionAndVelocity: Seq[(Int, Double, Double)], makeSpan: Int, schedule: Seq[OperationTimeSlot], tries: Int) {
  ...
  def calculateNewPositionAndVelocity(positionAndVelocity: Seq[(Int, Double, Double)], globalBestPosition: Seq[Double]): Seq[(Int, Double, Double)] = {
    val (jobIds, position, velocity): (Seq[Int], Seq[Double], Seq[Double]) = positionAndVelocity.unzip3
    val newVelocity: Seq[Double]                                           = calculateNewVelocity(position, velocity, globalBestPosition)
  ...
  def calculateNewVelocity(position: Seq[Double], velocity: Seq[Double], globalBestPosition: Seq[Double]): Seq[Double] = {
    val random: Random = Random
    val randomWeight   = random.nextDouble()

    val newVelocity: Seq[Double] = for ((operationVelocity, i) <- velocity.zipWithIndex) yield {
      val newOperationVelocity = operationVelocity + (randomWeight * (globalBestPosition(i) - position(i)))
